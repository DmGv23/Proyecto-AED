...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), sheet(new SparseMatrix())
{
    ...
    spnRow2 = new QSpinBox();
    spnRow2->setRange(1, ROWS);
    spnRow2->setValue(1);
    gRange->addWidget(spnRow2, 0, 1);

    gRange->addWidget(new QLabel("Col 2:"), 1, 0);
    spnCol2 = new QSpinBox();
    spnCol2->setRange(1, COLS);
    spnCol2->setValue(1);
...
void MainWindow::onInsertCell()
{
    ...
    if (val.isEmpty())
    {
        QMessageBox::warning(this, "Error", "Ingrese un valor.");
        return;
    }
...
void MainWindow::onAggregate()
{
    ...
    if (target == 0)
    {
        ...
        {
        case 0:
            result = sheet->sumRow(r1);
            desc = QString("SUMA fila %1").arg(r1 + 1);
            break;
        case 1:
            result = sheet->averageRow(r1);
            desc = QString("PROM fila %1").arg(r1 + 1);
            break;
        case 2:
            result = sheet->findMaxRow(r1);
            desc = QString("MAX fila %1").arg(r1 + 1);
            break;
        case 3:
            result = sheet->findMinRow(r1);
            desc = QString("MIN fila %1").arg(r1 + 1);
            break;
        }
    }
    else if (target == 1)
    {
        ...
        {
        case 0:
            result = sheet->sumColumn(c1);
            desc = QString("SUMA col %1").arg(colLetter(c1));
            break;
        case 1:
            result = sheet->averageColumn(c1);
            desc = QString("PROM col %1").arg(colLetter(c1));
            break;
        case 2:
            result = sheet->findMaxColumn(c1);
            desc = QString("MAX col %1").arg(colLetter(c1));
            break;
        case 3:
            result = sheet->findMinColumn(c1);
            desc = QString("MIN col %1").arg(colLetter(c1));
            break;
        }
    }
    else
    {
        ...
        case 0:
            ...
            desc = QString("SUMA rango");
            break;
        case 1:
            result = sheet->averageRange(r1, c1, r2, c2);
            desc = QString("PROM rango");
            break;
        case 2:
            result = sheet->maxRange(r1, c1, r2, c2);
            desc = QString("MAX rango");
            break;
        case 3:
            result = sheet->minRange(r1, c1, r2, c2);
            desc = QString("MIN rango");
            break;
        }
    }
